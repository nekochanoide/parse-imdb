...
class Range_slider(Frame):
    ...
    def __init__(self, master, width = 400, height = 80, min_val = 0, max_val = 1, init_lis = None, show_value = True):
        ...
        self.slider_x = Range_slider.BAR_RADIUS # x pos of the slider (left side)
    ...
    def __addTrack(self, startx, starty, endx, endy):
        id1 = self.canv.create_line(startx, starty, endx, endy, fill = Range_slider.LINE_COLOR, width = Range_slider.LINE_WIDTH)
        return id
    ...
    def __addBar(self, pos):
        ...
        R = Range_slider.BAR_RADIUS
        r = Range_slider.BAR_RADIUS_INNER
        L = self.canv_W - 2*self.slider_x
        y = self.slider_y
        x = self.slider_x+pos*L
        id_outer = self.canv.create_oval(x-R,y-R,x+R,y+R, fill = Range_slider.BAR_COLOR_OUTTER, width = 2, outline = "")
        id_inner = self.canv.create_oval(x-r,y-r,x+r,y+r, fill = Range_slider.BAR_COLOR_INNER, outline = "")
        if self.show_value:
            y_value = y+Range_slider.BAR_RADIUS+8
            value = pos*(self.max_val - self.min_val)+self.min_val
            id_value = self.canv.create_text(x,y_value, text = format(value, Range_slider.DIGIT_PRECISION))
    ...
    def __getValue(self, idx):
        ...
        bar = self.bars[idx]
        ids = bar["Ids"]
        x = self.canv.coords(ids[0])[0] + Range_slider.BAR_RADIUS
        pos = self.__calcPos(x)
